...
summary: 이 섹션에서는 NetApp XCP를 사용하여 GPFS를 구성하고 데이터를 NFS로 이동하는 데 필요한 자세한 단계를 제공합니다. 
---
= GPFS에서 NFS로의 자세한 단계
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
이 섹션에서는 NetApp XCP를 사용하여 GPFS를 구성하고 데이터를 NFS로 이동하는 데 필요한 자세한 단계를 제공합니다.



== GPFS 구성

. Linux용 Spectrum Scale Data Access를 서버 중 하나에 다운로드하여 설치합니다.
+
....
...
…
....
. 모든 노드에 필수 패키지(chef 및 커널 헤더 포함)를 설치합니다.
+
....
...
[root@mastr-51 installer]#
....
. 클러스터 정의 파일에 관리 노드와 GPFS 노드를 추가합니다.
+
....
...
....
. GPFS에서 사용할 원격 셸 바이너리를 지정합니다. `-r argument` .
+
....
...
[root@mastr-51 installer]#
....
. GPFS에서 사용할 원격 파일 복사 바이너리를 지정합니다. `-rc argument` .
+
....
...
[root@mastr-51 installer]#
....
. 모든 GPFS 노드에 설정할 포트 범위를 지정합니다. `-e argument` .
+
....
...
[root@hdp2 gpfstest]#
....
. XCP를 사용하여 GPFS에서 내보낸 NFS에서 NetApp NFS로 데이터를 이동합니다.
+
....
